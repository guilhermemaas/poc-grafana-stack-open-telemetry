...
def spans():
    """
    Request: http://service_c/lincros
    """

    #with tracer.start_as_current_span("Sites Aleatórios"):
    #    response = requests.get('https://www.terra.com.br')

    response = requests.get(f'{SERVICE_C_URL}/lincros')

    logger.info("Chamando serviço C", extra={"trace_id": format(trace.get_current_span().context.trace_id, '032x'), 
